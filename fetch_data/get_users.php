...
include('../include/db_connect.php');
	...
	$sub_array   = array();
   	$sub_array[] = $cou++;
   	$sub_array[] = $rows['user_name'];
   	$sub_array[] = $rows['user_email'];
   	$sub_array[] = date('d-m-Y',strtotime($rows['joining_date']));
	$sub_array[] = '<td>'.$active.'</td>';
   	$sub_array[] = '<td><a href="edit_user.php?id='.$rows["id"].'" id='.$rows["id"].' class="btn btn-outline-success btn-sm edit_data" data-target="">Edit</a> | <button id='.$rows["id"].' class="btn btn-outline-danger btn-sm delete_data">Delete</button></td>';
	$data[]      = $sub_array;
}
$output = array(
 "data"       =>  $data
