...
include('../include/db_connect.php');
	...
	$query = "SELECT * FROM company_bank_account ORDER BY id DESC";
	...
	$query_n = "SELECT * FROM company where id  = '".$rows['company']."'";
	$result_n = mysqli_query($conn, $query_n);
	$rows_n = mysqli_fetch_array($result_n);
	
	$sub_array   = array();
   	$sub_array[] = $cou++;
   	$sub_array[] = $rows_n['company_name'];
   	$sub_array[] = $rows['bank_name'];
   	$sub_array[] = $rows['account_number'];
   	$sub_array[] = $rows['account_type'];
